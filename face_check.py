from gtts import gTTS;import cv2,sys,numpy,os,playsound;file='haarcascade/haarcascade_frontalface_default.xml';img='image';images,labels,names,id=([],[],{},0);value=int(input("Enter check=1 , data=2 : "));
if value==1:
    for subdirs,dirs,files in os.walk(img):
        for subdir in dirs:
            names[id] = subdir;subjectpath = os.path.join(img, subdir)
            for filename in os.listdir(subjectpath):
                path=subjectpath+'/'+filename;label=id;images.append(cv2.imread(path,0));labels.append(int(label))
            id+=1
    width,height=(130,100);images,labels=[numpy.array(lis)for lis in [images,labels]];face_cascade=cv2.CascadeClassifier(file);cap=cv2.VideoCapture(1)
    while True:
    ...
    if not os.path.isdir(path):
        os.mkdir(path)
    width,height=(130,100);face_cascade=cv2.CascadeClassifier(file);cap=cv2.VideoCapture(1);count=1
    while count<10: 
